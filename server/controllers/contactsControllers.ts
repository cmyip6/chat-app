...
export class ContactsController {
    ...
    createContact = async (req: Request, res: Response) => {
		try {
            ...
            const check = await this.contactsService.checkContact(ownerId, targetId) 
            if (ownerId === targetId){
                res.json({
					success: false,
                ...
                const result = await this.contactsService.createContact(ownerId, targetId, nickname);
    ...
    getContactsList = async (req: Request, res: Response) => {
		try {
			...
			const contactsList = await this.contactsService.getContactsList(parseInt(userId));
			if (contactsList) {
				res.json({
					success: true,
